 ...
 */
	...
	if ( isset( $_GET[ 'mm_page' ] ) && !empty( $_GET[ 'mm_page' ] ) ) {
		if ( $_GET[ 'mm_page' ] == 'menu_g_blocks' ) {
			...
			echo "const mm_global_list_of_icons = [";
			if ( count( $icon_sets ) ) {
			...
			echo "const mm_global_list_of_animations = [";
			...
			echo "const mm_list_of_theme_menu_locations = [";
			...
			echo "const mm_list_of_nav_menus_structures = [";
			...
			echo mm_common::ntab(0) ."const mm_menu_config_vars = {";
...
*/
	...
	// check if 'mega_main' category exists.
