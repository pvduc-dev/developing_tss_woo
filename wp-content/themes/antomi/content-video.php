...
<article id="post-<?php the_ID(); ?>" <?php post_class($antomi_postclass); ?>>
	<div class="post-inner">
		...
		<?php if ( ! post_password_required() && ! is_attachment() ) : ?>
			<?php if ( is_single() ) { ?>
				<?php if (do_shortcode(get_post_meta( $post->ID, '_antomi_post_intro', true )) != '') { ?>
					<div class="post-thumbnail-wrapper">
						<div class="post-thumbnail">
							<?php echo do_shortcode(get_post_meta( $post->ID, '_antomi_post_intro', true )); ?>
						</div>
					</div>
				<?php } else { ?>
			...
			<div class="post-info">
				<?php if ( is_single() ) : ?>
					<div class="entry-content">
						<?php the_content( wp_kses(__( 'Continue reading <span class="meta-nav">&rarr;</span>', 'antomi' ), array('span'=>array('class'=>array())) )); ?>
						<?php wp_link_pages( array( 'before' => '<div class="page-links">' . esc_html__( 'Pages:', 'antomi' ), 'after' => '</div>', 'pagelink' => '<span>%</span>' ) ); ?>
					</div>
				<?php else : ?>
					<h2 class="post-title">
					...
					<?php if(get_the_author_meta()!="") { ?>
						<div class="author-info">
							...
							<div class="author-description">
								<h2><?php esc_html__( 'About the Author:', 'antomi'); printf( '<a href="'.esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ).'" rel="author">%s</a>' , get_the_author()); ?></h2>
