<?php
//WooCommerce Ajax
add_action('wp_head','antomi_woo_ajaxurl');
function antomi_woo_ajaxurl() {
?>
<?php
	...
	<div class="woocommerce product">
		...
		<div class="product-info">
			...
			<div class="price-box" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
				<p class="price"><?php echo wp_kses($product->get_price_html(), array( 
						'span' => array(
							'class' => array(),
						),
						'del' => array(),
			...
			<a class="see-all" href="<?php echo esc_url($product->get_permalink()); ?>"><?php echo esc_html($antomi_opt['quickview_link_text']); ?></a>
			<div class="quick-add-to-cart">
				<?php woocommerce_template_single_add_to_cart(); ?>
			</div>
			<div class="quick-desc"><?php echo do_shortcode(get_post($productid)->post_excerpt); ?></div>
			<?php if( function_exists('antomi_product_sharing') ) { ?>
				<div class="social-sharing"><?php antomi_product_sharing(); ?></div>
			<?php } ?>
		</div>
	</div>
	<?php
...
function get_cartinfo() {
	global $woocommerce;
	echo sprintf(_n('%d', '%d', $woocommerce->cart->cart_contents_count, 'antomi'), $woocommerce->cart->cart_contents_count);
...
function get_productinfo() {
	...
	$product = wc_get_product( $productid );
	?>
	<h3><?php esc_html_e('Product is added to cart', 'antomi');?></h3>
